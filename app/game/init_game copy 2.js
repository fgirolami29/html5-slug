(function() {
    ...
    function createLedges() {
        LEDGES.forEach(function(l){
            var ledge = platforms.create(l.x, l.y +90, 'ground');
    ...
    function create() {
        ...
        game.world.setBounds(0, 40, mission.width, mission.height);
        //We're going to be using physics, so enable the Arcade Physics system
        game.physics.startSystem(Phaser.Physics.ARCADE);

        game.add.sprite(0, 30, 'mission1');

        platforms = game.add.group();
        platforms.enableBody = true;

        ground = platforms.create(0, game.world.height , 'ground');
        ground.scale.setTo(4000, 1);
        ground.body.immovable = true;

        createLedges();

        player = game.add.sprite(50, game.world.height, 'marco');
